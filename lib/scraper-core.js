    ...
    async cluster({ urls, dataParser, scrapeRate = 5 }) {
        ...
        while (currentIndex < urls.length) {
            ...
            for (let i = 0; i < scrapeRate && currentIndex < urls.length; i++) {
                ...
                // create page and open for each url as scrapeRate allows
            ...
            for (const result of pages) {
                if (result.status === "fulfilled") {
                    const page = result.value; // result.value have page data, which is the page created in pagePromise
                    try {
                        const content = await page.content();
                        const parsedData = dataParser(content); // parse data using dataParser function which is passed as argument
                    ...
                    console.log(result)
                    console.error(
                        `Error creating page: ${result.reason.message}`
                    );
                }
            }
        }
